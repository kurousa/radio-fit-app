...
- [x] **タスク 1.2: Vite PWA プラグインの導入**
  ...
  - `vite-plugin-pwa`を開発依存関係として`package.json`に追加する (`pnpminstall vite-plugin-pwa -D`)。
...
- [x] **タスク 4.2: ローカルでの通知スケジューリング**
  ...
  - _注: 本来はサーバーからのPushイベントをトリガーにするのが一般的だが、今回はクライアントサイドのみで完結させる。_
...
- [x] **タスク 6.2: 動作確認**
  - 開発環境 (`pnpm dev`) とビルド後のプレビュー環境 (`pnpm build && pnpm preview`) の両方で、通知機能が意図通りに動作することを手動で確認する。
