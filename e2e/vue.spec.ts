...
test.describe('プロフィール画面の統計・モチベーション・通知設定', () => {
  test.beforeEach(async ({ page }) => {
    // localStorageをクリアして通知設定の初期状態を保証する
  ...
  test('通知設定のUIが正しく動作する', async ({ page, context }) => {
    // Notification.requestPermission を granted を返すようモック
    // （webkit では Notification API が未対応のため initScript でグローバルをモック）
