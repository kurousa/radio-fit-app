...
describe('TimezoneErrorHandler', () => {
  ...
  describe('handleError', () => {
    ...
    it('登録されたコールバック関数を呼び出す', () => {
      const mockCallback: Mock<(message: string, type: 'error' | 'warning' | 'info') => void> = vi.fn()
  ...
  describe('コールバック管理', () => {
    it('コールバック関数を正しく登録・削除する', () => {
      const mockCallback1: Mock<(message: string, type: 'error' | 'warning' | 'info') => void> = vi.fn()
      const mockCallback2: Mock<(message: string, type: 'error' | 'warning' | 'info') => void> = vi.fn()
